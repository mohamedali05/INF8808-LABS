...
def generate_data():
    ...
    return pd.DataFrame([(random.randint(1,99) , random.randint(1,99)) for i in range(0, m)] , columns= ['x' , 'y'])
...
def init_figure(dataframe, layout):
    ...
    return go.Figure(
        data=go.Scatter(x = dataframe['x'] , y = dataframe['y'], mode="markers" ,  marker=dict(color="#07BEB8", size=10)),
        layout=layout
    )
...
def update_label(figure):
    ...
    # Count the number of points (based on length of x or y values)
    num_points = len(figure.data[0].x)

    # Choose correct singular/plural text
    if num_points == 1:
        text = "Il y a "
        suffix = " point"
    else:
    ...
    # Return HTML element with number in bold
    return html.Span([
        text,
        html.B(str(num_points)),
        suffix
    ])
